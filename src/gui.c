...
static Clay_Vector2 nc__gui_window_to_clay_position(
...
static Clay_Vector2 nc__gui_touch_to_clay_position(
    const nc_gui_context_t* context,
    const float x,
    const float y
) {
    return (Clay_Vector2){
        x * (float)context->view.framebuffer_size.x,
        y * (float)context->view.framebuffer_size.y,
    };
}

static bool nc__gui_pointer_over_element(const Clay_Vector2 position) {
    Clay_SetPointerState(position, false);
    return Clay_GetPointerOverIds().length > 0 && !Clay_PointerOver(CLAY_ID("HUD"));
}
...
static void nc__gui_append_text(
    ...
    const float origin_x = command->boundingBox.x;
    float x = origin_x;
    float y = command->boundingBox.y;
...
static SDL_FRect nc__gui_rect_from_clay(const Clay_BoundingBox* box) {
    return (SDL_FRect){
        box->x,
        box->y,
        box->width,
...
static void nc__gui_build_draw_list(
    nc_gui_context_t* context,
        ...
        switch (command->commandType) {
            ...
            case CLAY_RENDER_COMMAND_TYPE_RECTANGLE:
                nc__gui_append_rectangle(
                        context,
                        &(nc_renderer_overlay_rectangle_t){
                            .rectangle = nc__gui_rect_from_clay(&command->boundingBox),
            ...
            case CLAY_RENDER_COMMAND_TYPE_SCISSOR_START: {
                ...
                SDL_FRect next = {
                    0.0f,
                    0.0f,
                    context->view.framebuffer_size.x,
                    context->view.framebuffer_size.y,
                };
                const SDL_FRect bounds = nc__gui_rect_from_clay(&command->boundingBox);
                if (command->renderData.clip.horizontal) {
                    next.x = bounds.x;
                    next.w = bounds.w;
                }
...
// Declares the touchscreen buttons and debug text to Clay. Analog sticks and the crosshair are
// procedural renderer passes and are not part of this layout.
...
bool nc_gui_handle_event(nc_gui_context_t* context, const SDL_Event* event) {
    ...
    switch (event->type) {
        ...
        case SDL_EVENT_MOUSE_BUTTON_DOWN:
            ...
            if (!nc__gui_pointer_over_element(nc__gui_window_to_clay_position(
                    context,
                    event->button.x,
                    event->button.y))) {
                return false;
        ...
        case SDL_EVENT_FINGER_DOWN:
            ...
            if (!nc__gui_pointer_over_element(nc__gui_touch_to_clay_position(
                    context,
                    event->tfinger.x,
                    event->tfinger.y))) {
                return false;
